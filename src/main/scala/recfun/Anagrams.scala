...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toList.groupBy((ch: Char) => ch).map { case (k, v) => (k, v.size)}.toList.sortWith(_._1 < _._1);
  }


  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    def createBigWort(s2: Sentence): Word = {
      if (!s2.isEmpty) s2.head + createBigWort(s2.tail)
      else ""
    }
    wordOccurrences(createBigWort(s))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def findNumber(pair: (Char, Int), y2: Occurrences): Int = {
      if (pair._1 == y2.head._1) pair._2 - y2.head._2
      else findNumber(pair, y2.tail)
    }
    x.map( el => if (y.exists( el2 => el2._1 == el._1)) (el._1, findNumber(el, y)) else el).filter(el => el._2 > 0);
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagrams2(freqs: Occurrences): List[Sentence] = {
      if (freqs.isEmpty) List(List())
      else {
        for {
          combination <- combinations(freqs)
          word <- dictionaryByOccurrences(combination)
          rest <- sentenceAnagrams2(subtract(freqs, combination))
        } yield word :: rest
      }
    }
